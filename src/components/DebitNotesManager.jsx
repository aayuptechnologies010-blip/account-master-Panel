...
import { Box, Input, Select, Table, Thead, Tbody, Tr, Th, Td, HStack, Button, IconButton, Badge } from '@chakra-ui/react';
...
export default function DebitNotesManager({ onDbChange }) {
  ...
  const loadData = async () => {
    try {
      const res = await apiService.getDebitNotes({ search, type }, page, pageSize);
      setNotes(res.notes || []);
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- refetch when search/type/page/pageSize change
    loadData();
      ...
      <Box {...card} overflow="hidden">
        <Box overflowX="auto">
          <Table variant="simple" size="sm" fontSize="13px">
            <Thead>
              <Tr>
                {['Voucher No', 'Type', 'Customer Name', 'Date', 'Credit Amount', 'Salesman', 'Actions'].map(h => (
                  <Th key={h} color="gray.500" fontSize="11px" fontWeight="700" bg="gray.50" textAlign={h === 'Actions' ? 'center' : 'left'}>{h}</Th>
                ))}
              </Tr>
            </Thead>
            <Tbody>
              {notes.length === 0 ? (
                <Tr><Td colSpan={7} textAlign="center" py={10} color="gray.400">No debit notes recorded.</Td></Tr>
              ) : notes.map(n => (
                <Tr key={n._id} _hover={{ bg: 'gray.50' }}>
                  <Td fontWeight="700" color="sky.700" fontFamily="mono" fontSize="12px">{n.voucherNo}</Td>
                  <Td>
                    <Badge borderRadius="20px" px={2.5} py={0.5} fontSize="11px" fontWeight="700" colorScheme={n.type === 'amount' ? 'purple' : 'orange'}>
                      {n.type === 'amount' ? 'Amount-Wise' : 'Item-Wise'}
                    </Badge>
                  </Td>
                  <Td fontWeight="600" color="gray.800">{n.customer?.partyName || '—'}</Td>
                  <Td color="gray.600">{new Date(n.date).toLocaleDateString()}</Td>
                  <Td fontWeight="800" color="gray.900">₹{n.amountR}</Td>
                  <Td color="gray.600">{n.salesmanName || 'N/A'}</Td>
                  <Td textAlign="center">
                    <HStack spacing={2} justify="center">
                      <Button size="xs" fontWeight="600" bg="sky.50" _hover={{ bg: 'sky.100' }} color="sky.700" borderRadius="8px" onClick={() => setViewDetail(n)}>Details</Button>
                      <IconButton aria-label="Delete Note" icon={<FaTrash />} size="sm" bg="rose.50" _hover={{ bg: 'rose.100' }} color="rose.600" onClick={() => handleDelete(n._id)} />
                    </HStack>
                  </Td>
                </Tr>
              ))}
