...
import { FaRupeeSign, FaUsers, FaBoxes, FaExclamationTriangle, FaArrowUp, FaArrowRight, FaCalendarDay, FaWalking, FaUserClock } from 'react-icons/fa';

const ACTIVITY_COLORS = { sale: '#0ea5e9', debit_note: '#a855f7', payment: '#22c55e' };
...
export default function Dashboard({ stats }) {
  ...
  useEffect(() => {
    ...
    loadActivity();
    const interval = setInterval(loadActivity, 45000);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      {/* Live Counters */}
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${apiService.isAdmin() ? 3 : 2}, 1fr)`, gap: '20px' }}>
        <StatCard label="Today's Sales" value={(stats.todaySales || 0).toLocaleString('en-IN')} icon={FaCalendarDay} iconBg="#fef3c7" iconColor="#d97706" borderColor="#f59e0b" prefix="₹" />
        <StatCard label="Salesmen Active Today" value={`${stats.activeSalesmenToday || 0} / ${stats.totalSalesmen || 0}`} icon={FaWalking} iconBg="#dcfce7" iconColor="#16a34a" borderColor="#22c55e" onClick={() => navigate('/salesmen')} />
        {apiService.isAdmin() && (
          <StatCard label="Pending Approvals" value={stats.pendingUsersCount ?? 0} icon={FaUserClock} iconBg="#fee2e2" iconColor="#dc2626" borderColor="#ef4444" onClick={() => navigate('/users')} alert={stats.pendingUsersCount > 0} />
        )}
      </div>
      ...
      {/* Live Activity Feed */}
      <SectionCard title="Live Activity" subtitle="Recent invoices, credits & payments across your team">
        <div style={{ maxHeight: '360px', overflowY: 'auto', padding: '8px 0' }}>
          {activity.length === 0 ? (
            <div style={{ textAlign: 'center', color: '#94a3b8', padding: '32px', fontSize: '13px' }}>No recent activity yet.</div>
          ) : activity.map((a, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', padding: '10px 20px' }}>
              <div style={{
                width: '8px', height: '8px', borderRadius: '50%', marginTop: '5px', flexShrink: 0,
                background: ACTIVITY_COLORS[a.type] || '#94a3b8'
              }} />
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: '13px', color: '#334155', fontWeight: '500' }}>{a.message}</div>
                <div style={{ fontSize: '11px', color: '#94a3b8', marginTop: '2px' }}>{timeAgo(a.timestamp)}</div>
              </div>
            </div>
          ))}
        </div>
