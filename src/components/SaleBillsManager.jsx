...
import { Box, Input, Select, Table, Thead, Tbody, Tr, Th, Td, HStack, Button, IconButton, Text } from '@chakra-ui/react';
...
export default function SaleBillsManager({ onDbChange }) {
  ...
  const loadData = async () => {
    try {
      const res = await apiService.getSaleBills({ search }, page, pageSize);
      setBills(res.bills || []);
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- refetch when search/page/pageSize change
    loadData();
      ...
      <Box {...card} overflow="hidden">
        <Box overflowX="auto">
          <Table variant="simple" size="sm" fontSize="13px">
            ...
            <Tbody>
              {bills.length === 0 ? (
                <Tr><Td colSpan={7} textAlign="center" py={10} color="gray.400">No sale bills found.</Td></Tr>
              ) : bills.map(b => (
                <Tr key={b._id} _hover={{ bg: 'gray.50' }}>
                  <Td fontWeight="700" color="sky.700" fontFamily="mono" fontSize="12px">{b.voucherNo}</Td>
                  <Td>
                    <Text fontWeight="600" color="gray.800">{b.customer?.partyName || '—'}</Text>
                    <Text fontSize="11px" color="gray.400">Area: {b.area || 'N/A'}</Text>
                  </Td>
                  <Td color="gray.600">{new Date(b.date).toLocaleDateString()}</Td>
                  <Td fontWeight="800" color="sky.500">₹{b.amountR}</Td>
                  <Td color="gray.600">{b.salesmanName || 'N/A'}</Td>
                  <Td color="gray.600">{b.qty}</Td>
                  <Td textAlign="center">
                    <HStack spacing={2} justify="center">
                      <Button size="xs" fontWeight="600" bg="sky.50" _hover={{ bg: 'sky.100' }} color="sky.700" borderRadius="8px" onClick={() => setViewDetail(b)}>View</Button>
                      <IconButton aria-label="Delete Bill" icon={<FaTrash />} size="sm" bg="rose.50" _hover={{ bg: 'rose.100' }} color="rose.600" onClick={() => handleDelete(b._id)} />
                    </HStack>
                  </Td>
                </Tr>
              ))}
